(function (window) {
    ...
    window.onscroll = function () {
        ...
        if(checkWillLoadImage('pic')){
            createPic();
            waterFall('waterFall','pic');
        }
    };

    $("input_commit").onmousedown  = function () {
        var pwd = $("pwd").value;
            ...
            jQuery.post( "http://115.159.215.229:8080/telenet/little-cute/user",
                {
                    ...
                    if(result === "user not exists"){
                        alert("the user not exists,enter again");
                        $("pwd").value ="";
                        $("uname").value= "";
                    }
                })
    ...
    $("register_commit").onmousedown  = function () {
        var pwd = $("pwd").value;
        var name = $("uname").value;
            ...
            jQuery.post("http://115.159.215.229:8080/telenet/little-cute/user",
                {
                    ...
                    if(result=== "user already existed"){
                        alert("user already exist,change a name");
                        $("pwd").value ="";
                        $("uname").value= "";
                    }
                })
 ...
 */
function login() {
    $('login_button').onmousedown = function () {
 ...
 */
function tab() {
    var lis = $('class_head').getElementsByTagName("li");
    var doms = $('tab_content_in').children;
...
function adsAutoPlay() {
    var lis = $('ads').getElementsByTagName('li');
    var index = 0;
    setInterval(function () {
        for (var i = 0; i < lis.length; i++) {
            var cli = lis[i];
            buffer(cli, {opacity: 0}, null);
        }
...
function createPic() {
    ...
    var newstr = $('waterFall').innerHTML;
    ...
    for (var i = 0; i < 30; i++) {
        var str =
            ...
            "<div class=\'pic-box\' >" +
            "<img src='" + json[i].pic + "'>" +
            "<div class=\'imgCover\'" + "id='imgCover'>" +
            "<div class=\'pic-btns\'>" +
            "<a href=\'\' class=\'collet\'>采集</a>" +
            "<a href=''  class='like'>" +
            "<span></span>" +
            "</a>" +
            "</div>" +
    ...
    for(var i = 0; i<boxes.length; i++){
        boxes[i].onmouseover = function () {
            this.childNodes[0].lastChild.style.display = 'block';
        }
        boxes[i].onmouseout = function () {
            this.childNodes[0].lastChild.style.display = 'none';
        }
    }
}
